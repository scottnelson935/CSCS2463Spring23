...
let spriteSheetFilenames = ["SpelunkyGuy.png", "Cyclops.png", "Green.png"]
let spriteSheets = [];
// let totalAnimations = 8;
let animations = [];
...
function reset() {
  ...
  for (let i = 0; i < game.totalSprites; i++) {
    animations[i] = new WalkingAnimation(random(spriteSheets),80,80,random(100,300),random(100,300),8, random(1, 3),6, random([0,1]));
  }  
}
...
function draw() {
  switch(game.state) {
    ...
    case GameState.GameOver:
      ...
      text("Press any key to continue!", 200, 375);
      break;
    case GameState.Start:
      ...
      textSize(50);
      textAlign(CENTER);
      text("Cyclops Game", width/2, height/2);
      textSize(25);
...
class WalkingAnimation {
  ...
  draw() {
    ...
    // rect(-this.sw/3, -this.sh/2.5, this.sw/1.5, this.sh/1.3);
    ...
    if(this.vertical) {
      ...
      this.move(this.dx,((this.sw /2)-15), width - ((this.sw /2)-15))
    }
  }
  ...
  contains(x,y) {
    ...
    let insideY = y >= this.dy - 35 && y <= this.dy + 70;
